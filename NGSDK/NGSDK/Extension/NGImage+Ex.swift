...
extension UIImage {
    ...
    func resetSizeOfImageData(maxSize: NSInteger) -> Data? {
        ...
        var finalImageData = self.jpegData(compressionQuality: 1.0)
        ...
        // 获取原图片宽高比
        let sourceImageAepectRatio = self.size.width / self.size.height
        // 先调整分辨率
        var defaultSize = CGSize(width: 1024, height: 1024 / sourceImageAepectRatio)
        guard let newImage = newSizeImage(defaultSize, sourceImage: self) else { return nil }
